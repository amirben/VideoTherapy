...
namespace VideoTherapy.Kinect
{
    class DrawSkeleton
    {
        ...
        /// </summary>
        private const double ClipBoundsThickness = 6;
        ...
        public ImageSource DrawBodySkeleton(BodyFrame bodyFrame)
        {
            if (body == null)
            {
                body = new Body[bodyFrame.BodyCount];
            }

            bodyFrame.GetAndRefreshBodyData(body);

            UpdateBodyFrame(body);
            ...
            if (bodies != null)
            {
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    foreach (Body body in bodies)
                    {
                        ...
                        if (body.IsTracked)
                        {
                            //If the body is in the edges of the image
                            ...
                            //Draw the hand status - open, close and etc. not needed for now
                            //this.DrawHand(body.HandLeftState, jointPoints[JointType.HandLeft], dc);
                            //this.DrawHand(body.HandRightState, jointPoints[JointType.HandRight], dc);

                            //use for tracking only one person
                            break;
                        }
                    }
